<?php
/**
 * Template Name: Word Stack Page copy
 *
...
<style>
    ...
    .wide-container {
        ...
        gap: 20px; /* Add some space between the words */
...
<script>
    ...
    const kerningPairs = {
        'IS': 20, 'YO': -110, // Adjust the value as needed
        'UT': 35, 'OW': -40, // Add more pairs as needed
        'TU': -30, 'NO': 10, // Add more pairs as needed
    };

    // Function to apply kerning
    ...
    // Function to generate combined SVG
    function generateCombinedSVG(letters, combinedViewBox, wordClass) {
        const svgNS = "http://www.w3.org/2000/svg";
        const svg = document.createElementNS(svgNS, "svg");
        svg.setAttribute("viewBox", combinedViewBox);
        svg.setAttribute("xmlns", svgNS);
        svg.setAttribute("class", wordClass); // Add the class for the word
        svg.setAttribute("height", "100%"); // Set the height to 100% to fill the container
        svg.style.opacity = "1"; // Keep opacity as 1
        ...
        letters.forEach(letter => {
            ...
            if (letterData) {
                ...
                totalWidth += parseInt(letterData.viewBox.split(' ')[2]) + 20;
            } else {
        ...
        // Apply kerning
        applyKerning(groups, kerningPairs, 20);

        // Set the width based on the aspect ratio and fixed height
        const aspectRatio = totalWidth / 800;
        svg.setAttribute("width", `${200 * aspectRatio}px`);
    ...
    function displayWords(words) {
        ...
        wideContainer.innerHTML = ""; // Clear previous SVGs

        words.forEach(word => {
            const totalWidth = word.split('').reduce((acc, letter) => acc + parseInt(svgData[letter.toUpperCase()].viewBox.split(' ')[2]) + 20, 0);
            const combinedViewBox = `0 0 ${totalWidth} 800`;
            const svgElement = generateCombinedSVG(word.split(''), combinedViewBox, `word-svg word-${word.replace(/\s/g, '-').toLowerCase()}`);
            wideContainer.appendChild(svgElement);
        });

        // Start the intro and sliding animation
    ...
    function animateIntroAndSlide() {
        ...
        const padding = 20; // Padding between the containers

        // Clone the wide container
        const clonedContainer = wideContainer.cloneNode(true);
        clonedContainer.style.left = `${totalWidth + padding}px`; // Position to the right of the first container with padding
        svgContainer.appendChild(clonedContainer);

        // Animate the sliding with GSAP
        const slideTimeline = gsap.timeline({ repeat: -1, ease: "none" })
            .to([wideContainer, clonedContainer], { x: -totalWidth - padding, duration: 20, force3D: true, ease: "none" })
            .set(wideContainer, { x: totalWidth + padding })
            .set(clonedContainer, { x: totalWidth + padding });

        // Animate the words into view one by one while sliding
    ...
    // Example usage: Display the sentence "when your future is now"
    displayWords([ "        ","when", "your", "future", "is", "now", "  "]);
</script>
<?php
get_footer();
?>
